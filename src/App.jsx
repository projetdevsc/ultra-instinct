...
export default function App(){
  ...
  const getNutriBefore=(date,days=3)=>{
    ...
    const result=[];
    for(let i=1;i<=days;i++){const dd=new Date(d);dd.setDate(dd.getDate()-i);const key=dd.toISOString().slice(0,10);const m=allMacros.find(x=>x.date===key);if(m)result.push(m)}
  ...
  const generateDailyReport=()=>{
    ...
    r+=`\n## 🏋️ Dernières séances\n`;
    rs.forEach(s=>{
      const rt=INIT_ROUTINES[s.routine];const nb=getNutriBefore(s.date);const nl=nutriLevel(nb);
    ...
    return(<div style={shell}><style>{css}</style>
      ...
      <div style={{padding:"0 16px",paddingBottom:80}}>
        ...
        {sTab==="chrono"&&<div>
          ...
          {sessHist.map((s,i)=>{const r=INIT_ROUTINES[s.routine];const nb=getNutriBefore(s.date);const nl=nutriLevel(nb);return(
