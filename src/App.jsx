...
export default function App(){
  ...
  useEffect(()=>{if(rKey&&scr==="workout"){const exs=INIT_ROUTINES[rKey].exercises.slice();setExOrder(exs);setSlotKeys(exs.map(()=>"s"+(slotCounter.current++)))}},[rKey,scr]);
  ...
  const generateDailyReport=()=>{
    ...
    r+=`\n## 🏋️ Dernières séances\n`;rs.forEach(s=>{const rt=INIT_ROUTINES[s.routine];r+=`${dateFR(s.date)} · ${rt?.emoji} ${rt?.name} · ${s.duration} · ${s.prs} PR${s.prs>1?"s":""}\n`});
    ...
    <div style={{position:"relative",zIndex:1}}>
      ...
      <div style={{padding:"0 16px",marginBottom:16}}>
        <div style={{...card,textAlign:"center",background:T.am,border:"1px solid rgba(112,144,255,0.15)",animation:"fadeUp 0.4s ease both"}}>
          ...
          <div style={{fontSize:sz(11,fSc),color:T.t3,fontWeight:600,marginTop:2}}>jours avant l'objectif · 27/04/2027</div></div></div>
      {/* Daily summary */}
    ...
    return(<div style={shell}><style>{css}</style>
      ...
      <div style={{padding:"0 16px",paddingBottom:80}}>
        ...
        {sTab==="chrono"&&<div>
          ...
          {sessHist.map((s,i)=>{const r=INIT_ROUTINES[s.routine];return(
        ...
        {/* History by routine */}
        {sTab==="routine"&&<div>{Object.entries(grouped).map(([rk,sessions],gi)=>{const r=INIT_ROUTINES[rk];return(
  ...
  if(scr==="body"){
    ...
    const handleAddMensur=()=>{const entry={date:formData.date};MENSUR_FIELDS.forEach(f=>{if(formData[f.key]&&formData[f.key]!=="")entry[f.key]=parseFloat(formData[f.key])});
      if(Object.keys(entry).length>1){setMensur(DB.addMensuration(entry));setShowForm(false);setFormData({date:new Date().toISOString().slice(0,10),poids:"",bf:"",ventre:"",poitrine:"",cuisse:"",bras:"",muscle:"",meta:""})}};
    return(<div style={shell}><style>{css}</style>
