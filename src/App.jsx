...
/* ═══════════════════════════════════════════════════
   ULTRA INSTINCT — V7 BIG ITERATION
   Fix T order · Reorder exos · Custom exo · Settings
   ═══════════════════════════════════════════════════ */
...
/* ─── THEME (declared BEFORE anything that references it) ─── */
...
/* ─── MENSUR FIELDS & DATA (AFTER T — this was the page-blank bug) ─── */
const INIT_MENSUR=[
...
function RestTimer({duration,exName,onDone,onSkip,vibOn}){
  ...
  useEffect(()=>{setRem(duration);clearInterval(iR.current);
    iR.current=setInterval(()=>{setRem(p=>{if(p<=1){clearInterval(iR.current);if(vibOn&&navigator.vibrate)navigator.vibrate([200,100,200,100,300]);cbR.current();return 0}return p-1})},1000);
...
function ExCard({exId,alts,onRest,nSets,swaps,onSwap,onData,customObjs,onObjChange,vibOn}){
  ...
  const validate=i=>{const n=[...sets];n[i].done=true;setSets(n);if(prR&&n[i].kg>prR.kg){setPrs(p=>[...p,i]);if(vibOn&&navigator.vibrate)navigator.vibrate([100,50,100,50,200])}onRest(rest,ax.name)};
  ...
  const setObjMode=(mode)=>{
    ...
    setObjInput(text);onObjChange(aId,{mode,text});setShowObj(false);
  };
...
function Nav({active,go}){return(<div style={{position:"fixed",bottom:0,left:0,right:0,zIndex:50,background:"rgba(13,18,32,0.96)",backdropFilter:"blur(24px)",borderTop:`1px solid ${T.bdM}`,display:"flex",padding:"6px 0 calc(8px + env(safe-area-inset-bottom, 12px))"}}>
  {[{id:"home",i:"⚡",l:"Home"},{id:"stats",i:"📊",l:"Stats"},{id:"history",i:"📋",l:"Historique"},{id:"body",i:"📐",l:"Corps"},{id:"settings",i:"⚙️",l:"Réglages"}].map(x=><div key={x.id} onClick={()=>go(x.id)} style={{flex:1,textAlign:"center",cursor:"pointer",padding:"4px 0"}}><div style={{fontSize:18,marginBottom:2}}>{x.i}</div><div style={{fontSize:9,fontWeight:700,letterSpacing:"0.8px",color:active===x.id?T.blL:T.t4}}>{x.l}</div></div>)}
</div>)}
...
export default function App(){
  ...
  useEffect(()=>{if(rKey&&scr==="workout")setExOrder(INIT_ROUTINES[rKey].exercises.slice())},[rKey,scr]);
  ...
  const moveEx=(idx,dir)=>{setExOrder(prev=>{const n=[...prev];const t=idx+dir;if(t<0||t>=n.length)return prev;[n[idx],n[t]]=[n[t],n[idx]];return n})};
  const addExToSession=(exId)=>{setExOrder(p=>[...p,exId]);setShowAddEx(false);setAddExSearch("");setNewExName("");setNewExMuscle("")};
  ...
  const removeEx=(idx)=>{setExOrder(p=>p.filter((_,i)=>i!==idx))};
  ...
  const finish=()=>{
    ...
    const session={id:Date.now(),routine:rKey,date:today,duration:dur,exercises:Object.keys(workoutData).length||exOrder.length,prs};
  ...
  const reset=()=>{setScr("home");setRKey(null);setStart(null);setElapsed(0);setWorkoutData({});setReportText("");setSessionNotes("");setExOrder([]);setShowAddEx(false)};
  ...
  const shell={fontFamily:"'Outfit',-apple-system,sans-serif",background:T.bg,color:T.w,minHeight:"100vh",paddingBottom:"calc(76px + env(safe-area-inset-bottom, 0px))",position:"relative",fontSize:`${fSc}rem`};
          ...
          {Object.entries(INIT_ROUTINES).map(([key,r],idx)=>{const ld=lastDates[key];return(
            <button key={key} onClick={()=>goR(key)} style={{padding:"20px 16px 16px",borderRadius:16,cursor:"pointer",background:T.bgCard,border:`1px solid ${T.bd}`,textAlign:"left",position:"relative",overflow:"hidden",animation:`fadeUp 0.4s ease ${idx*0.08}s both`}}>
              ...
              <div style={{position:"relative"}}>
                ...
                {ld&&<div style={{fontSize:10,color:T.t4,marginTop:6,fontWeight:600,display:"flex",alignItems:"center",gap:4}}><span style={{color:T.vi}}>●</span> {ago(ld)}</div>}
              </div></button>)})}
        </div></div>
  ...
  if(scr==="workout"){
    ...
    <div style={{padding:"14px 12px",paddingBottom:timer?150:80}}>
      ...
      {exOrder.map((exId,i)=>(
        <div key={exId+"-"+i} style={{animation:`fadeUp 0.35s ease ${i*0.05}s both`}}>
          ...
          <ExCard exId={exId} alts={routine.alts[exId]} onRest={tRest} nSets={routine.sets} swaps={swaps} onSwap={handleSwap} onData={handleExData} customObjs={customObjs} onObjChange={handleObjChange} vibOn={vibOn}/>
        </div>))}
      {!showAddEx&&<button onClick={()=>setShowAddEx(true)} style={{width:"100%",padding:"14px",marginTop:8,background:"rgba(112,144,255,0.06)",border:"1.5px dashed rgba(112,144,255,0.25)",borderRadius:14,color:T.blL,fontSize:13,fontWeight:700,cursor:"pointer"}}>+ Ajouter un exercice</button>}
    ...
    {timer&&<RestTimer duration={timer.duration} exName={timer.exerciseName} onDone={()=>setTimer(null)} onSkip={()=>setTimer(null)} vibOn={vibOn}/>}
  </div>)}
    ...
    <div style={{position:"relative",zIndex:1}}>
      ...
      <div style={{padding:"0 16px"}}>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:8,marginBottom:16,animation:"fadeUp 0.4s ease 0.2s both"}}>
          {[{l:"Durée",v:fm(elapsed),c:T.bl},{l:"Exercices",v:Object.keys(workoutData).length||exOrder.length,c:T.vi},{l:"PRs",v:prCount,c:T.pk}].map((s,i)=>(<div key={i} style={{padding:"18px 10px",borderRadius:14,textAlign:"center",background:T.bgCard,border:`1px solid ${T.bd}`}}><div style={{fontSize:22,fontWeight:900,color:s.c}}>{s.v}</div><div style={{fontSize:10,color:T.t4,marginTop:5,fontWeight:600}}>{s.l}</div></div>))}</div>
    ...
    <div style={{padding:"0 16px",paddingBottom:80}}>
      {histView==="chrono"&&sessHist.map((s,i)=>{const r=INIT_ROUTINES[s.routine];return(
        <div key={s.id} style={{padding:"14px 16px",borderRadius:14,marginBottom:8,background:T.bgCard,border:`1px solid ${T.bd}`,animation:`fadeUp 0.3s ease ${i*0.03}s both`,display:"flex",justifyContent:"space-between",alignItems:"center"}}>
          <div><div style={{display:"flex",alignItems:"center",gap:8}}><span style={{fontSize:16}}>{r?.emoji}</span><span style={{fontSize:14,fontWeight:700,color:T.w}}>{r?.name}</span></div>
          <div style={{fontSize:11,color:T.t3,marginTop:3}}>{dateFR(s.date)} · {s.duration} · {s.exercises} exos</div></div>
          <div style={{textAlign:"right"}}>{s.prs>0&&<div style={{fontSize:12,fontWeight:800,color:T.pk}}>🏆 {s.prs} PR{s.prs>1?"s":""}</div>}</div></div>)})}
      {histView==="routine"&&Object.entries(grouped).map(([rk,sessions],gi)=>{const r=INIT_ROUTINES[rk];return(
        <div key={rk} style={{marginBottom:16,animation:`fadeUp 0.3s ease ${gi*0.05}s both`}}>
          <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:8,padding:"0 4px"}}><span style={{fontSize:18}}>{r?.emoji}</span><span style={{fontSize:15,fontWeight:800,color:T.w}}>{r?.name}</span><span style={{fontSize:11,color:T.t3}}>({sessions.length} séances)</span></div>
          {sessions.map((s,i)=>(<div key={s.id} style={{padding:"10px 16px",borderRadius:12,marginBottom:6,background:T.bgCard,border:`1px solid ${T.bd}`,display:"flex",justifyContent:"space-between",alignItems:"center"}}>
            <div style={{fontSize:12,color:T.t2}}>{dateFR(s.date)} · {s.duration}</div>
            <div style={{display:"flex",alignItems:"center",gap:8}}><span style={{fontSize:11,color:T.t3}}>{s.exercises} exos</span>{s.prs>0&&<span style={{fontSize:11,fontWeight:800,color:T.pk}}>🏆 {s.prs}</span>}</div></div>))}</div>)})}
      ...
      <div style={{padding:"0 16px",paddingBottom:80}}>
        {sTab==="prs"&&<div>{prBoard.map((it,i)=>(<div key={it.id} onClick={()=>setSelEx(selEx===it.id?null:it.id)} style={{padding:"12px 16px",borderRadius:14,marginBottom:8,cursor:"pointer",background:selEx===it.id?T.bgEl:T.bgCard,border:`1px solid ${selEx===it.id?T.bdM:T.bd}`,animation:`fadeUp 0.3s ease ${i*0.03}s both`}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
            ...
            <div style={{textAlign:"right"}}><div style={{fontSize:16,fontWeight:900,color:T.pk}}>{it.pr.kg}<span style={{fontSize:11,fontWeight:500,color:T.t3}}>kg</span> <span style={{fontSize:12,color:T.t2}}>× {it.pr.r}</span></div>{it.gain>0&&<div style={{fontSize:10,fontWeight:700,color:T.cy,marginTop:2}}>+{it.gain}kg</div>}</div></div>
        ...
        {sTab==="progress"&&<div>{selEx?(<div>
          ...
          <div style={{padding:"16px",borderRadius:16,background:T.bgCard,border:`1px solid ${T.bd}`}}>
            ...
            <div style={{marginTop:16}}>{[...EX[selEx].hist].reverse().map((h,i)=>(<div key={i} style={{display:"flex",justifyContent:"space-between",padding:"7px 0",borderBottom:`1px solid ${T.bd}`}}><span style={{fontSize:12,color:T.t2}}>{dateFR(h.d)}</span><span style={{fontSize:13,fontWeight:700,color:i===0?T.pk:T.t1}}>{h.kg}kg × {h.r}</span></div>))}</div></div>
    ...
    return(<div style={shell}><style>{css}</style>
      ...
      {showForm&&<div style={{padding:"0 16px",marginBottom:16,animation:"fadeUp 0.3s ease both"}}>
        <div style={{padding:"16px",borderRadius:16,background:T.bgCard,border:`1px solid ${T.bdM}`}}>
          ...
          <button onClick={handleAddMensur} style={{width:"100%",marginTop:14,padding:"12px",borderRadius:10,cursor:"pointer",background:"rgba(112,144,255,0.1)",border:"1.5px solid rgba(112,144,255,0.28)",color:T.blL,fontSize:14,fontWeight:700}}>✓ Enregistrer</button></div></div>}
      <div style={{display:"flex",padding:"0 16px",gap:6,marginBottom:16}}>
        {[{id:"dashboard",l:"📊 Vue"},{id:"history",l:"📋 Historique"}].map(t=>(<button key={t.id} onClick={()=>setBodyTab(t.id)} style={{flex:1,padding:"9px 8px",borderRadius:10,cursor:"pointer",fontSize:12,fontWeight:700,background:bodyTab===t.id?"rgba(112,144,255,0.1)":"rgba(180,200,255,0.03)",border:`1px solid ${bodyTab===t.id?"rgba(112,144,255,0.25)":T.bd}`,color:bodyTab===t.id?T.blL:T.t3}}>{t.l}</button>))}</div>
      <div style={{padding:"0 16px",paddingBottom:80}}>
        {bodyTab==="dashboard"&&<div>
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
            {MENSUR_FIELDS.map((f,i)=>{const val=latest?.[f.key];const d=delta(f.key);const hist=sparkData(f.key);if(!val&&hist.length===0)return null;return(
              <div key={f.key} style={{padding:"14px 12px",borderRadius:14,background:T.bgCard,border:`1px solid ${T.bd}`,animation:`fadeUp 0.3s ease ${i*0.04}s both`}}>
                ...
                <div style={{fontSize:22,fontWeight:900,color:f.color}}>{val}<span style={{fontSize:12,fontWeight:500,color:T.t3}}> {f.unit}</span></div>
          ...
          {mensur.length>=2&&<div style={{marginTop:16,padding:"16px",borderRadius:14,background:T.bgCard,border:`1px solid ${T.bd}`,animation:"fadeUp 0.3s ease 0.3s both"}}>
            ...
            {MENSUR_FIELDS.filter(f=>{const first=mensur.find(m=>m[f.key]!=null);const last2=mensur.slice().reverse().find(m=>m[f.key]!=null);return first&&last2&&first!==last2}).map(f=>{
              ...
              return(<div key={f.key} style={{display:"flex",justifyContent:"space-between",padding:"6px 0",borderBottom:`1px solid ${T.bd}`}}><span style={{fontSize:12,color:T.t2}}>{f.label}</span><span style={{fontSize:12,fontWeight:800,color:good?T.cy:T.wa}}>{diff>0?"+":""}{diff.toFixed(1)} {f.unit}</span></div>)})}</div>}
        </div>}
        {bodyTab==="history"&&<div>{[...mensur].reverse().map((m,i)=>(
          <div key={i} style={{padding:"14px 16px",borderRadius:14,marginBottom:8,background:T.bgCard,border:`1px solid ${T.bd}`,animation:`fadeUp 0.3s ease ${i*0.03}s both`}}>
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:6}}>
              {MENSUR_FIELDS.filter(f=>m[f.key]!=null).map(f=>(<div key={f.key} style={{textAlign:"center"}}><div style={{fontSize:9,color:T.t4,fontWeight:600}}>{f.icon}</div><div style={{fontSize:14,fontWeight:800,color:f.color}}>{m[f.key]}</div><div style={{fontSize:8,color:T.t4}}>{f.unit}</div></div>))}</div></div>))}</div>}
      </div><Nav active="body" go={setScr}/></div>)}

  /* ═══ SETTINGS ═══ */
  if(scr==="settings"){
    const[showReset,setShowReset]=useState(false);
    const toggleVib=()=>setSettings(p=>({...p,vibration:!p.vibration}));
    const setFS=(sz)=>setSettings(p=>({...p,fontSize:sz}));
    const handleReset=()=>{DB.resetAll();setSwaps({});setCustomObjs({});setSessHist(INIT_HISTORY);setLastDates({...LAST_SESSIONS});setMensur([...INIT_MENSUR]);setSettings({vibration:true,fontSize:"normal"});setShowReset(false)};
    return(<div style={shell}><style>{css}</style>
      <div style={{padding:`calc(16px + ${safeTop}) 20px 16px`}}><Logo sz="sm"/><h2 style={{margin:"12px 0 0",fontSize:22,fontWeight:800,color:T.w}}>Réglages</h2></div>
      <div style={{padding:"0 16px",paddingBottom:80}}>
        {/* Vibration */}
        <div style={{padding:"16px",borderRadius:16,background:T.bgCard,border:`1px solid ${T.bd}`,marginBottom:10,animation:"fadeUp 0.3s ease both"}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
            <div><div style={{fontSize:14,fontWeight:700,color:T.w}}>📳 Vibration</div><div style={{fontSize:11,color:T.t3,marginTop:3}}>Vibrer en fin de timer et sur les PRs</div></div>
            <button onClick={toggleVib} style={{width:52,height:30,borderRadius:15,cursor:"pointer",background:settings.vibration?"rgba(112,144,255,0.2)":"rgba(180,200,255,0.06)",border:`1px solid ${settings.vibration?"rgba(112,144,255,0.35)":T.bd}`,position:"relative",transition:"all 0.3s",padding:0}}>
              <div style={{width:24,height:24,borderRadius:12,background:settings.vibration?T.bl:T.t4,position:"absolute",top:2,left:settings.vibration?26:2,transition:"all 0.3s",boxShadow:settings.vibration?"0 0 8px rgba(112,144,255,0.4)":"none"}}/></button></div></div>
        {/* Font size */}
        <div style={{padding:"16px",borderRadius:16,background:T.bgCard,border:`1px solid ${T.bd}`,marginBottom:10,animation:"fadeUp 0.3s ease 0.05s both"}}>
          <div style={{fontSize:14,fontWeight:700,color:T.w,marginBottom:10}}>🔤 Taille de police</div>
          <div style={{display:"flex",gap:6}}>
            {[{id:"small",l:"Petit"},{id:"normal",l:"Normal"},{id:"large",l:"Grand"}].map(s=>(<button key={s.id} onClick={()=>setFS(s.id)} style={{flex:1,padding:"10px 8px",borderRadius:10,cursor:"pointer",background:settings.fontSize===s.id?"rgba(112,144,255,0.12)":"rgba(180,200,255,0.03)",border:`1px solid ${settings.fontSize===s.id?"rgba(112,144,255,0.3)":T.bd}`,color:settings.fontSize===s.id?T.blL:T.t3,fontSize:12,fontWeight:700}}>{s.l}</button>))}</div></div>
        {/* Reset */}
        <div style={{padding:"16px",borderRadius:16,background:T.bgCard,border:`1px solid ${T.bd}`,marginBottom:10,animation:"fadeUp 0.3s ease 0.1s both"}}>
          <div style={{fontSize:14,fontWeight:700,color:T.w,marginBottom:4}}>🗑️ Réinitialiser les données</div>
          <div style={{fontSize:11,color:T.t3,marginBottom:12}}>Supprime historique, swaps, objectifs custom et mensurations</div>
          {!showReset?<button onClick={()=>setShowReset(true)} style={{padding:"10px 20px",borderRadius:10,cursor:"pointer",background:"rgba(226,128,255,0.06)",border:"1px solid rgba(226,128,255,0.2)",color:T.pk,fontSize:12,fontWeight:700}}>Réinitialiser</button>
        ...
        {/* About */}
        <div style={{padding:"16px",borderRadius:16,background:T.bgCard,border:`1px solid ${T.bd}`,marginBottom:10,animation:"fadeUp 0.3s ease 0.15s both"}}>
          <div style={{fontSize:14,fontWeight:700,color:T.w,marginBottom:10}}>ℹ️ À propos</div>
          {[{l:"Application",v:"Ultra Instinct"},{l:"Version",v:`v${APP_VERSION}`},{l:"Stack",v:"React 18 · PWA"},{l:"Hébergement",v:"Vercel"},{l:"Données",v:"localStorage (local)"},{l:"Auteur",v:"Stephen C."}].map((r,i)=>(
            <div key={i} style={{display:"flex",justifyContent:"space-between",padding:"4px 0",borderBottom:`1px solid ${T.bd}`}}><span style={{fontSize:12,color:T.t3}}>{r.l}</span><span style={{fontSize:12,fontWeight:600,color:T.t1}}>{r.v}</span></div>))}</div>
