...
function ExCard({exId,alts,onRest,nSets,swaps,onSwap}){
  const swapped=swaps[exId];
  const[aId,setAId]=useState(swapped||exId);
  const ax=EX[aId];const lp=last(aId);const prR=pr(aId);const obj=OBJ[aId];
  ...
  return(<div style={{background:allD?"linear-gradient(135deg,rgba(92,232,250,0.04),rgba(112,144,255,0.03))":T.bgCard,border:`1px solid ${allD?"rgba(92,232,250,0.12)":T.bd}`,borderRadius:16,marginBottom:10,overflow:"hidden"}}>
    <div onClick={()=>setOpen(!open)} style={{padding:"13px 16px",cursor:"pointer",display:"flex",justifyContent:"space-between",alignItems:"center",borderBottom:open?`1px solid ${T.bd}`:"none"}}>
      ...
      <div style={{fontSize:11,color:T.t3,marginTop:3}}>{ax.muscle}{lp?` · ${lp.kg}kg × ${lp.r}`:""}{obj?<span style={{color:T.vi}}> · 🎯 {obj}</span>:""}</div></div>
...
export default function App(){
  ...
  const[swaps,setSwaps]=useState({}); // {exId: swappedToId} — persistent swap memory
  const[sessHist,setSessHist]=useState(INIT_HISTORY);
  const[lastDates,setLastDates]=useState({...LAST_SESSIONS});
  ...
  const goR=k=>{setRKey(k);setStart(Date.now());setElapsed(0);setScr("workout")};
  const finish=()=>{clearInterval(ref.current);setTimer(null);
    const today=new Date().toISOString().slice(0,10);
    setLastDates(p=>({...p,[rKey]:today}));
    setSessHist(p=>[{id:Date.now(),routine:rKey,date:today,duration:fm(elapsed),exercises:INIT_ROUTINES[rKey].exercises.length,prs:0},...p]);
    setScr("summary")};
  const reset=()=>{setScr("home");setRKey(null);setStart(null);setElapsed(0)};
  ...
  if(scr==="workout"){return(<div style={shell}><style>{css}</style>
    ...
    <div style={{padding:"14px 12px",paddingBottom:timer?150:80}}>
      ...
      {routine.exercises.map((exId,i)=><div key={exId} style={{animation:`fadeUp 0.35s ease ${i*0.05}s both`}}>
        <ExCard exId={exId} alts={routine.alts[exId]} onRest={tRest} nSets={routine.sets} swaps={swaps} onSwap={handleSwap}/></div>)}</div>
    ...
    <div style={{position:"relative",zIndex:1}}>
      ...
      <div style={{padding:"0 16px"}}>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:8,marginBottom:16,animation:"fadeUp 0.4s ease 0.2s both"}}>
          {[{l:"Durée",v:fm(elapsed),c:T.bl},{l:"Exercices",v:routine.exercises.length,c:T.vi},{l:"Volume",v:`~${routine.exercises.length*4}`,s:"sets",c:T.cy}].map((s,i)=>(<div key={i} style={{padding:"18px 10px",borderRadius:14,textAlign:"center",background:T.bgCard,border:`1px solid ${T.bd}`}}>
            <div style={{fontSize:22,fontWeight:900,color:s.c}}>{s.v}{s.s&&<span style={{fontSize:11,fontWeight:500,color:T.t3,marginLeft:2}}>{s.s}</span>}</div>
            <div style={{fontSize:10,color:T.t4,marginTop:5,fontWeight:600}}>{s.l}</div></div>))}</div>
        <div style={{padding:"22px 20px",borderRadius:16,textAlign:"center",background:T.am,border:"1px solid rgba(112,144,255,0.15)",marginBottom:14,animation:"fadeUp 0.4s ease 0.3s both"}}>
          ...
          <div style={{fontSize:12,color:T.t2,marginBottom:14,lineHeight:1.6}}>Génère ton rapport et colle-le dans Claude pour un débrief</div>
          <button style={{padding:"12px 28px",borderRadius:10,cursor:"pointer",background:"rgba(112,144,255,0.1)",border:"1.5px solid rgba(112,144,255,0.28)",color:T.blL,fontSize:14,fontWeight:700}}>📋 Copier le rapport</button></div>
    ...
    return(<div style={shell}><style>{css}</style>
      ...
      <div style={{padding:"0 16px",paddingBottom:80}}>
        ...
        {sTab==="volume"&&<div>
          ...
          <div style={{marginTop:20,fontSize:10,fontWeight:800,color:T.t4,letterSpacing:"2px",textTransform:"uppercase",marginBottom:12}}>Mensurations (01/02/2026)</div>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
            {[{l:"Poids",v:"73,25 kg",o:"78-80 kg",c:T.bl},{l:"Body Fat",v:"11,9%",o:"12-14%",c:T.cy},{l:"Poitrine",v:"104 cm",o:"108-110 cm",c:T.pk},{l:"Cuisses",v:"58 cm",o:"61-63 cm",c:T.vi},{l:"Bras",v:"39 cm",o:"40-41 cm",c:T.wa},{l:"Ventre",v:"74,5 cm ✓",o:"< 78 cm",c:T.cy}].map((s,i)=>(<div key={i} style={{padding:"14px 12px",borderRadius:12,background:T.bgCard,border:`1px solid ${T.bd}`,animation:`fadeUp 0.3s ease ${i*0.04}s both`}}>
              <div style={{fontSize:10,color:T.t3,fontWeight:600,marginBottom:4}}>{s.l}</div><div style={{fontSize:18,fontWeight:900,color:s.c}}>{s.v}</div><div style={{fontSize:10,color:T.vi,fontWeight:600,marginTop:4}}>🎯 {s.o}</div></div>))}</div>
        </div>}
