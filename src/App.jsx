...
function ExCard({exId,alts,onRest,nSets,swaps,onSwap,onData}){
  ...
  // Report data up to parent whenever sets change
  useEffect(()=>{
    if(onData){
...
export default function App(){
  ...
  const goR=k=>{setRKey(k);setStart(Date.now());setElapsed(0);setWorkoutData({});setPrCount(0);setScr("workout")};
  ...
  const handleCopyReport=async()=>{
    const ok=await copyReport(reportText);
    setCopied(ok);
    setTimeout(()=>setCopied(false),2500);
  };
  
  const reset=()=>{setScr("home");setRKey(null);setStart(null);setElapsed(0);setWorkoutData({});setReportText("")};
  ...
  if(scr==="workout"){return(<div style={shell}><style>{css}</style>
    ...
    <div style={{padding:"14px 12px",paddingBottom:timer?150:80}}>
      ...
      {routine.exercises.map((exId,i)=><div key={exId} style={{animation:`fadeUp 0.35s ease ${i*0.05}s both`}}>
        <ExCard exId={exId} alts={routine.alts[exId]} onRest={tRest} nSets={routine.sets} swaps={swaps} onSwap={handleSwap} onData={handleExData}/></div>)}</div>
    ...
    <div style={{position:"relative",zIndex:1}}>
      ...
      <div style={{padding:"0 16px"}}>
        ...
        <div style={{padding:"22px 20px",borderRadius:16,textAlign:"center",background:T.am,border:"1px solid rgba(112,144,255,0.15)",marginBottom:14,animation:"fadeUp 0.4s ease 0.3s both"}}>
          ...
          <div style={{fontSize:12,color:T.t2,marginBottom:14,lineHeight:1.6}}>Copie le rapport et colle-le dans Claude pour un débrief personnalisé</div>
          <button onClick={handleCopyReport} style={{padding:"12px 28px",borderRadius:10,cursor:"pointer",background:copied?"rgba(92,232,250,0.15)":"rgba(112,144,255,0.1)",border:`1.5px solid ${copied?"rgba(92,232,250,0.35)":"rgba(112,144,255,0.28)"}`,color:copied?T.cy:T.blL,fontSize:14,fontWeight:700,transition:"all 0.3s"}}>{copied?"✅ Copié !":"📋 Copier le rapport"}</button>
          {reportText&&<div style={{marginTop:12,textAlign:"left",padding:"12px",borderRadius:10,background:"rgba(0,0,0,0.2)",maxHeight:200,overflowY:"auto"}}>
            <pre style={{margin:0,fontSize:10,color:T.t3,whiteSpace:"pre-wrap",fontFamily:"monospace",lineHeight:1.5}}>{reportText.slice(0,500)}{reportText.length>500?"...":""}</pre>
          </div>}</div>
