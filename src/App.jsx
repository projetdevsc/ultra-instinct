...
/* ═══════════════════════════════════════════════════════════
   ULTRA INSTINCT — V8
   Quotidien (eau+macros) · Countdown · Détail historique
   Suppr séance · Fix reorder · Fix font · Export/Import
   ═══════════════════════════════════════════════════════════ */
...
const sz=(base,scale)=>Math.round(base*scale);
...
function ExCard({exId,slotKey,alts,onRest,nSets,swaps,onSwap,onData,customObjs,onObjChange,fSc}){
  ...
  const lp=last(aId);const prR=pr(aId);
  ...
  useEffect(()=>{
    if(onData){
      ...
      onData(slotKey,{activeId:aId,name:ax.name,muscle:ax.muscle,sets:sets.map(s=>({kg:s.kg,reps:s.reps,done:s.done,isPR:s.kg>0&&prR&&s.kg>prR.kg})),bestSet,prevBest:lp,hasPR:prs.length>0,objective:obj});
    }
  },[sets,prs]);

  const validate=i=>{const n=[...sets];n[i].done=true;setSets(n);if(prR&&n[i].kg>prR.kg)setPrs(p=>[...p,i]);onRest(rest,ax.name)};
  const swap=nId=>{setAId(nId);onSwap(exId,nId);setRest(EX[nId]?.rest||120);setSets(Array(nSets).fill(null).map(()=>({kg:0,reps:0,done:false})));setPrs([]);setShowSwap(false);setSearch("");
    ...
    {showObj&&<div onClick={e=>e.stopPropagation()} style={{padding:"12px 16px",background:"rgba(168,140,255,0.04)",borderBottom:`1px solid ${T.bd}`}}>
      ...
      <div style={{display:"flex",gap:6,marginBottom:10,flexWrap:"wrap"}}>
        {[{id:"up",label:"📈 Augmenter",desc:lp?`${lp.kg+5}kg × ${lp.r}`:"+5kg"},{id:"stable",label:"🔒 Stabiliser",desc:lp?`${lp.kg}kg × ${lp.r+2}`:"+2 reps"},{id:"default",label:"🏠 V6",desc:defaultObj||"—"}].map(m=>
          <button key={m.id} onClick={()=>setObjMode(m.id)} style={{flex:"1 0 80px",padding:"8px 6px",borderRadius:9,cursor:"pointer",textAlign:"center",background:objMode===m.id?"rgba(168,140,255,0.12)":"rgba(180,200,255,0.03)",border:`1px solid ${objMode===m.id?"rgba(168,140,255,0.25)":T.bd}`,color:objMode===m.id?T.vi:T.t3}}>
            <div style={{fontSize:12,fontWeight:700}}>{m.label}</div><div style={{fontSize:10,color:T.t4,marginTop:2}}>{m.desc}</div></button>)}
      </div>
      <div style={{display:"flex",gap:6}}>
        <input type="text" value={objInput} onChange={e=>setObjInput(e.target.value)} placeholder="Ex: 150kg × 8" style={{flex:1,padding:"9px 12px",borderRadius:9,background:T.bgInput,border:`1px solid ${T.bdM}`,color:T.w,fontSize:13,outline:"none",fontFamily:"inherit"}}/>
...
function Nav({active,go}){return(<div style={{position:"fixed",bottom:0,left:0,right:0,zIndex:50,background:"rgba(13,18,32,0.96)",backdropFilter:"blur(24px)",borderTop:`1px solid ${T.bdM}`,display:"flex",padding:"6px 0 calc(8px + env(safe-area-inset-bottom, 12px))"}}>
  {[{id:"home",i:"⚡",l:"Home"},{id:"daily",i:"💧",l:"Quotidien"},{id:"stats",i:"📊",l:"Stats"},{id:"history",i:"📋",l:"Historique"},{id:"body",i:"📐",l:"Corps"},{id:"settings",i:"⚙️",l:"Réglages"}].map(x=><div key={x.id} onClick={()=>go(x.id)} style={{flex:1,textAlign:"center",cursor:"pointer",padding:"4px 0"}}><div style={{fontSize:16,marginBottom:2}}>{x.i}</div><div style={{fontSize:8,fontWeight:700,letterSpacing:"0.5px",color:active===x.id?T.blL:T.t4}}>{x.l}</div></div>)}
</div>)}

function Logo({sz:s}){const lg=s==="lg";return(<div><div style={{fontSize:lg?34:17,fontWeight:900,letterSpacing:lg?"1px":"0.5px",lineHeight:1,background:T.ss,WebkitBackgroundClip:"text",WebkitTextFillColor:"transparent",filter:`drop-shadow(0 0 ${lg?30:14}px rgba(192,208,255,0.25))`}}>ULTRA INSTINCT</div>{lg&&<div style={{marginTop:10,fontSize:11,fontWeight:600,letterSpacing:"5px",textTransform:"uppercase",color:T.t3,textAlign:"center"}}>Beyond your limits</div>}</div>)}
...
export default function App(){
  /* ─── ALL useState at top level (React hooks rule) ─── */
  ...
  useEffect(()=>{
    if(rKey&&scr==="workout"){
      const exs=INIT_ROUTINES[rKey].exercises.slice();
      setExOrder(exs);
      setSlotKeys(exs.map((_,i)=>"slot_"+(slotCounter.current++)));
    }
  },[rKey,scr]);
  ...
  const handleExData=(slotKey,data)=>{setWorkoutData(p=>({...p,[slotKey]:data}))};

  const moveEx=(idx,dir)=>{
    const t=idx+dir;
    setExOrder(p=>{const n=[...p];if(t<0||t>=n.length)return p;[n[idx],n[t]]=[n[t],n[idx]];return n});
    setSlotKeys(p=>{const n=[...p];if(t<0||t>=p.length)return p;[n[idx],n[t]]=[n[t],n[idx]];return n});
  };
  const addExToSession=(exId)=>{setExOrder(p=>[...p,exId]);setSlotKeys(p=>[...p,"slot_"+(slotCounter.current++)]);setShowAddEx(false);setAddExSearch("");setNewExName("");setNewExMuscle("")};
  ...
  const finish=()=>{
    ...
    const today=new Date().toISOString().slice(0,10);const dur=fm(elapsed);
    const prs=Object.values(workoutData).filter(d=>d.hasPR).length;setPrCount(prs);
    // Build detail for history
    ...
    const newHist=DB.addSession(session);setSessHist(newHist);
    const newDates=DB.setLastDate(rKey,today);setLastDates(newDates);
    ...
    const r=INIT_ROUTINES[rKey];const exResults=Object.values(workoutData).filter(d=>d.sets.some(s=>s.done));
    setReportText(generateReport(r.name,r.emoji,dur,exResults,new Date().toLocaleDateString("fr-FR"),""));
    setScr("summary");
  };
  const handleCopyReport=async()=>{
    const r=INIT_ROUTINES[rKey];const exResults=Object.values(workoutData).filter(d=>d.sets.some(s=>s.done));
    const fresh=generateReport(r.name,r.emoji,fm(elapsed),exResults,new Date().toLocaleDateString("fr-FR"),sessionNotes);
  ...
  // Water handlers
  const addWater=(ml)=>{const updated=DB.addWater(ml);setWaterToday(updated)};
  const undoWater=()=>{const updated=DB.removeLastWater();setWaterToday(updated)};
  const recentAmounts=DB.getRecentWaterAmounts();

  // Macros handler
  const saveMacros=()=>{
    const data={kcal:parseFloat(macroForm.kcal)||0,prot:parseFloat(macroForm.prot)||0,gluc:parseFloat(macroForm.gluc)||0,lip:parseFloat(macroForm.lip)||0};
    const saved=DB.setMacrosToday(data);setMacrosToday(saved);
  };

  // Delete session
  const deleteSession=(id)=>{const updated=DB.deleteSession(id);setSessHist(updated);setConfirmDel(null)};
  ...
  const doExport=()=>{
    ...
    const a=document.createElement("a");a.href=url;a.download=`ultra-instinct-backup-${new Date().toISOString().slice(0,10)}.json`;
    document.body.appendChild(a);a.click();document.body.removeChild(a);URL.revokeObjectURL(url);
    DB.setLastExport();setSettings(s=>({...s}));
  };
  const doImport=(e)=>{
    const file=e.target.files[0];if(!file)return;
    const reader=new FileReader();
    reader.onload=(ev)=>{try{const data=JSON.parse(ev.target.result);DB.importAll(data);window.location.reload()}catch{alert("Fichier invalide")}};
    reader.readAsText(file);
  };

  // Daily report
  const generateDailyReport=()=>{
    const wh=DB.getWaterHistory(7);const mh=DB.getMacrosHistory(7);
    const latestM=mensur.length>0?mensur[mensur.length-1]:null;
    const recentSess=sessHist.slice(0,4);
    let r=`# 📊 Ultra Instinct — Rapport Hebdo\n**Date** : ${new Date().toLocaleDateString("fr-FR")}\n**Countdown** : ${daysLeft()} jours avant l'objectif\n\n`;
    r+=`## 💧 Hydratation (7j)\n`;wh.forEach(d=>{r+=`${dateFR(d.date)} : ${(d.total/1000).toFixed(1)}L\n`});
    r+=`\n## 🍎 Macros (7j)\n`;mh.forEach(d=>{r+=`${dateFR(d.date)} : ${d.kcal}kcal · P${d.prot}g · G${d.gluc}g · L${d.lip}g\n`});
    if(latestM){r+=`\n## 📐 Mensurations (${dateFR(latestM.date)})\n`;MENSUR_FIELDS.forEach(f=>{if(latestM[f.key]!=null)r+=`${f.label} : ${latestM[f.key]} ${f.unit}\n`})}
    r+=`\n## 🏋️ Dernières séances\n`;recentSess.forEach(s=>{const rt=INIT_ROUTINES[s.routine];r+=`${dateFR(s.date)} · ${rt?.emoji} ${rt?.name} · ${s.duration} · ${s.prs} PR${s.prs>1?"s":""}\n`});
    r+=`\n---\n*Généré par Ultra Instinct v${APP_VERSION}*`;
    return r;
  };
  const copyDailyReport=async()=>{const r=generateDailyReport();const ok=await copyReport(r);setCopied(ok);setTimeout(()=>setCopied(false),2500)};
  ...
  const btn=(active)=>({flex:1,padding:"9px 8px",borderRadius:10,cursor:"pointer",fontSize:sz(12,fSc),fontWeight:700,background:active?"rgba(112,144,255,0.1)":"rgba(180,200,255,0.03)",border:`1px solid ${active?"rgba(112,144,255,0.25)":T.bd}`,color:active?T.blL:T.t3});
  ...
  if(scr==="home"){
    ...
    <div style={{position:"relative",zIndex:1}}>
      <div style={{padding:`calc(20px + ${safeTop}) 24px 20px`,textAlign:"center"}}>
      ...
      <div style={{padding:"0 16px",marginBottom:16}}>
        <div style={{...card,textAlign:"center",background:T.am,border:"1px solid rgba(112,144,255,0.15)",animation:"fadeUp 0.4s ease both"}}>
          <div style={{fontSize:sz(32,fSc),fontWeight:900,background:T.ss,WebkitBackgroundClip:"text",WebkitTextFillColor:"transparent"}}>{daysLeft()}</div>
          <div style={{fontSize:sz(11,fSc),color:T.t3,fontWeight:600,marginTop:2}}>jours avant l'objectif · 27/04/2027</div>
        </div>
      </div>

      {/* Mini daily summary */}
      <div style={{padding:"0 16px",marginBottom:16,display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
        <div style={{...card,animation:"fadeUp 0.4s ease 0.05s both"}} onClick={()=>setScr("daily")}>
          <div style={{fontSize:9,fontWeight:800,color:T.cy,letterSpacing:"1px",textTransform:"uppercase",marginBottom:6}}>💧 Eau</div>
          <div style={{fontSize:sz(18,fSc),fontWeight:900,color:T.cy}}>{(waterToday.total/1000).toFixed(1)}<span style={{fontSize:sz(11,fSc),color:T.t3}}>/{(waterGoal/1000).toFixed(0)}L</span></div>
          <div style={{height:4,borderRadius:2,background:"rgba(92,232,250,0.08)",marginTop:6,overflow:"hidden"}}><div style={{height:"100%",borderRadius:2,background:T.cy,width:`${wPct}%`}}/></div>
        </div>
        <div style={{...card,animation:"fadeUp 0.4s ease 0.1s both"}} onClick={()=>setScr("daily")}>
          <div style={{fontSize:9,fontWeight:800,color:T.wa,letterSpacing:"1px",textTransform:"uppercase",marginBottom:6}}>🍎 Macros</div>
          {macrosToday?<div>
            <div style={{fontSize:sz(18,fSc),fontWeight:900,color:T.wa}}>{macrosToday.kcal}<span style={{fontSize:sz(11,fSc),color:T.t3}}> kcal</span></div>
            <div style={{fontSize:sz(10,fSc),color:T.t3,marginTop:2}}>P{macrosToday.prot}g · G{macrosToday.gluc}g · L{macrosToday.lip}g</div>
          </div>:<div style={{fontSize:sz(12,fSc),color:T.t4,fontWeight:600}}>Non renseigné</div>}
        </div>
      </div>
      ...
      <div style={{padding:"0 16px"}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
          {Object.entries(INIT_ROUTINES).map(([key,r],idx)=>{const ld=lastDates[key];return(
            <button key={key} onClick={()=>goR(key)} style={{padding:"20px 16px 16px",borderRadius:16,cursor:"pointer",background:T.bgCard,border:`1px solid ${T.bd}`,textAlign:"left",position:"relative",overflow:"hidden",animation:`fadeUp 0.4s ease ${0.15+idx*0.08}s both`}}>
  ...
  if(scr==="daily"){
    const wPct=Math.min(waterToday.total/waterGoal*100,100);
    const wHist=DB.getWaterHistory(7);
    const maxW=Math.max(...wHist.map(d=>d.total),waterGoal);
    return(<div style={shell}><style>{css}</style>
    ...
    <div style={{display:"flex",padding:"0 16px",gap:6,marginBottom:16}}>
      {[{id:"water",l:"💧 Eau"},{id:"macros",l:"🍎 Macros"},{id:"report",l:"📋 Rapport"}].map(t=><button key={t.id} onClick={()=>setDailyTab(t.id)} style={btn(dailyTab===t.id)}>{t.l}</button>)}
    </div>
    <div style={{padding:"0 16px",paddingBottom:80}}>
      ...
      {dailyTab==="water"&&<div>
        ...
        <div style={{...card,textAlign:"center",marginBottom:12,animation:"fadeUp 0.3s ease both"}}>
          <div style={{fontSize:sz(42,fSc),fontWeight:900,color:T.cy}}>{(waterToday.total/1000).toFixed(1)}<span style={{fontSize:sz(16,fSc),color:T.t3}}>L</span></div>
          <div style={{fontSize:sz(11,fSc),color:T.t3,marginTop:2}}>{Math.round(wPct)}% · objectif {(waterGoal/1000).toFixed(0)}L</div>
          <div style={{height:8,borderRadius:4,background:"rgba(92,232,250,0.08)",marginTop:12,overflow:"hidden"}}><div style={{height:"100%",borderRadius:4,background:wPct>=100?"linear-gradient(90deg,#5CE8FA,#8AE8A0)":T.cy,width:`${wPct}%`,transition:"width 0.3s"}}/></div>
        </div>

        {/* Quick add */}
        <div style={{display:"flex",gap:6,marginBottom:12,flexWrap:"wrap"}}>
          {(recentAmounts.length>0?recentAmounts:[250,500,750]).map(ml=>(
            <button key={ml} onClick={()=>addWater(ml)} style={{flex:"1 0 60px",padding:"12px 8px",borderRadius:10,cursor:"pointer",textAlign:"center",background:"rgba(92,232,250,0.06)",border:"1px solid rgba(92,232,250,0.15)",color:T.cy,fontSize:sz(14,fSc),fontWeight:700}}>{ml}ml</button>
          ))}
        </div>

        {/* Custom input */}
        <div style={{display:"flex",gap:6,marginBottom:12}}>
          <input type="number" inputMode="numeric" placeholder="Volume en ml" value={waterInput} onChange={e=>setWaterInput(e.target.value)}
            style={{flex:1,padding:"10px 12px",borderRadius:9,background:T.bgInput,border:`1px solid ${T.bdM}`,color:T.w,fontSize:sz(14,fSc),outline:"none",fontFamily:"inherit"}}/>
          <button onClick={()=>{const v=parseInt(waterInput);if(v>0){addWater(v);setWaterInput("")}}} style={{padding:"10px 20px",borderRadius:9,cursor:"pointer",background:"rgba(92,232,250,0.1)",border:"1px solid rgba(92,232,250,0.25)",color:T.cy,fontSize:sz(13,fSc),fontWeight:700}}>+ Eau</button>
        </div>
          ...
          <div style={{display:"flex",alignItems:"flex-end",gap:6,height:100}}>
            {wHist.map((d,i)=>{const pct=maxW>0?(d.total/maxW)*100:0;const day=new Date(d.date+"T12:00:00").toLocaleDateString("fr-FR",{weekday:"short"}).charAt(0).toUpperCase();return(
              <div key={i} style={{flex:1,textAlign:"center"}}>
                <div style={{fontSize:9,fontWeight:700,color:d.total>=waterGoal?T.cy:T.t3,marginBottom:4}}>{(d.total/1000).toFixed(1)}</div>
                <div style={{height:70,borderRadius:4,background:"rgba(92,232,250,0.06)",display:"flex",alignItems:"flex-end",overflow:"hidden"}}>
                  <div style={{width:"100%",height:`${pct}%`,borderRadius:4,background:d.total>=waterGoal?"linear-gradient(180deg,#5CE8FA,#8AE8A0)":"rgba(92,232,250,0.3)",transition:"height 0.3s"}}/>
                </div>
                <div style={{fontSize:9,fontWeight:700,color:T.t4,marginTop:4}}>{day}</div>
              </div>)})}
          </div>
        </div>
      </div>}
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
            {[{k:"kcal",l:"Calories",u:"kcal",c:T.wa},{k:"prot",l:"Protéines",u:"g",c:T.pk},{k:"gluc",l:"Glucides",u:"g",c:T.bl},{k:"lip",l:"Lipides",u:"g",c:T.vi}].map(f=>(
              <div key={f.k}><label style={{fontSize:10,color:T.t3,fontWeight:600,display:"block",marginBottom:3}}>{f.l} ({f.u})</label>
                <input type="number" inputMode="decimal" value={macroForm[f.k]} onChange={e=>setMacroForm(p=>({...p,[f.k]:e.target.value}))}
                  placeholder={macrosToday?.[f.k]?.toString()||""} style={{width:"100%",padding:"10px",borderRadius:9,background:T.bgInput,border:`1px solid ${T.bdM}`,color:f.c,fontSize:sz(16,fSc),fontWeight:700,outline:"none",fontFamily:"inherit",textAlign:"center"}}/></div>))}
          </div>
          <button onClick={saveMacros} style={{width:"100%",marginTop:14,padding:"12px",borderRadius:10,cursor:"pointer",background:"rgba(255,184,110,0.1)",border:"1.5px solid rgba(255,184,110,0.28)",color:T.wa,fontSize:sz(14,fSc),fontWeight:700}}>✓ Enregistrer</button>
        </div>
        {macrosToday&&<div style={{...card,animation:"fadeUp 0.3s ease 0.05s both"}}>
          ...
          <div style={{display:"flex",justifyContent:"space-around"}}>
            {[{v:macrosToday.kcal,l:"kcal",c:T.wa},{v:macrosToday.prot,l:"P",c:T.pk},{v:macrosToday.gluc,l:"G",c:T.bl},{v:macrosToday.lip,l:"L",c:T.vi}].map((x,i)=>(
              <div key={i} style={{textAlign:"center"}}><div style={{fontSize:sz(20,fSc),fontWeight:900,color:x.c}}>{x.v}</div><div style={{fontSize:9,color:T.t3}}>{x.l}</div></div>))}
          </div>
        </div>}
      </div>}
      ...
      {dailyTab==="report"&&<div>
        <div style={{...card,animation:"fadeUp 0.3s ease both"}}>
          ...
          <div style={{fontSize:sz(12,fSc),color:T.t2,marginBottom:12,lineHeight:1.6}}>Compile eau, macros, mensurations et séances des 7 derniers jours</div>
      ...
      {exOrder.map((exId,i)=>(
        <div key={slotKeys[i]} style={{animation:`fadeUp 0.35s ease ${i*0.05}s both`}}>
          <div style={{display:"flex",justifyContent:"flex-end",gap:4,marginBottom:2,paddingRight:4}}>
            ...
            <button onClick={()=>removeEx(i)} style={{width:26,height:22,borderRadius:6,cursor:"pointer",background:"rgba(226,128,255,0.06)",border:"1px solid rgba(226,128,255,0.15)",color:T.pk,fontSize:11,fontWeight:700,display:"flex",alignItems:"center",justifyContent:"center",padding:0}}>✕</button>
          </div>
          <ExCard exId={exId} slotKey={slotKeys[i]} alts={routine.alts[exId]} onRest={tRest} nSets={routine.sets} swaps={swaps} onSwap={handleSwap} onData={handleExData} customObjs={customObjs} onObjChange={handleObjChange} fSc={fSc}/>
        </div>))}
    ...
    <div style={{position:"relative",zIndex:1}}>
      ...
      <div style={{padding:"0 16px"}}>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:8,marginBottom:16,animation:"fadeUp 0.4s ease 0.2s both"}}>
          {[{l:"Durée",v:fm(elapsed),c:T.bl},{l:"Exercices",v:Object.keys(workoutData).length||exOrder.length,c:T.vi},{l:"PRs",v:prCount,c:T.pk}].map((s,i)=>(<div key={i} style={{padding:"18px 10px",borderRadius:14,textAlign:"center",...card}}><div style={{fontSize:sz(22,fSc),fontWeight:900,color:s.c}}>{s.v}</div><div style={{fontSize:10,color:T.t4,marginTop:5,fontWeight:600}}>{s.l}</div></div>))}</div>
        <div style={{padding:"22px 20px",borderRadius:16,background:T.am,border:"1px solid rgba(112,144,255,0.15)",marginBottom:14,animation:"fadeUp 0.4s ease 0.3s both"}}>
          ...
          <textarea value={sessionNotes} onChange={e=>setSessionNotes(e.target.value)} placeholder={"Tes ressentis, questions, ajustements...\nEx: Hack squat facile → monter à 150kg ?\nDouleur épaule droite sur le bench..."} rows={4} style={{width:"100%",padding:"12px",borderRadius:10,background:T.bgInput,border:`1px solid ${T.bdM}`,color:T.w,fontSize:sz(13,fSc),outline:"none",fontFamily:"inherit",resize:"vertical",lineHeight:1.6,marginBottom:12,minHeight:80}}/>
  ...
  if(scr==="history"){
    ...
    return(<div style={shell}><style>{css}</style>
    <div style={{padding:`calc(16px + ${safeTop}) 20px 16px`}}><Logo sz="sm"/><h2 style={{margin:"12px 0 0",fontSize:sz(22,fSc),fontWeight:800,color:T.w}}>Historique</h2></div>
    <div style={{display:"flex",padding:"0 16px",gap:6,marginBottom:16}}>
      {[{id:"chrono",l:"📅 Chronologique"},{id:"routine",l:"🏋️ Par routine"}].map(t=><button key={t.id} onClick={()=>setHistView(t.id)} style={btn(histView===t.id)}>{t.l}</button>)}</div>
    <div style={{padding:"0 16px",paddingBottom:80}}>
      {/* Session detail modal */}
      {histDetail&&<div style={{...card,marginBottom:12,animation:"fadeUp 0.2s ease both",border:`1px solid ${T.bdM}`}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}>
          <div style={{fontSize:sz(15,fSc),fontWeight:800,color:T.w}}>{INIT_ROUTINES[histDetail.routine]?.emoji} {INIT_ROUTINES[histDetail.routine]?.name} · {dateFR(histDetail.date)}</div>
          <button onClick={()=>setHistDetail(null)} style={{padding:"4px 10px",borderRadius:7,cursor:"pointer",background:"rgba(180,200,255,0.04)",border:`1px solid ${T.bd}`,color:T.t3,fontSize:11,fontWeight:700}}>✕</button>
        </div>
        <div style={{fontSize:sz(11,fSc),color:T.t3,marginBottom:10}}>{histDetail.duration} · {histDetail.exercises} exos · {histDetail.prs} PR{histDetail.prs>1?"s":""}</div>
        {histDetail.detail?histDetail.detail.map((d,i)=>(
          <div key={i} style={{display:"flex",justifyContent:"space-between",padding:"6px 0",borderBottom:`1px solid ${T.bd}`}}>
            ...
            <div style={{display:"flex",alignItems:"center",gap:6}}>
              <span style={{fontSize:sz(13,fSc),fontWeight:700,color:d.hasPR?T.pk:T.t1}}>{d.bestKg}kg × {d.bestReps}</span>
              {d.hasPR&&<span style={{fontSize:8,fontWeight:800,color:T.pk}}>🏆</span>}
            </div>
          </div>))
        :<div style={{fontSize:sz(11,fSc),color:T.t4}}>Détail non disponible pour cette séance</div>}
      </div>}
      ...
      {histView==="chrono"&&sessHist.map((s,i)=>{const r=INIT_ROUTINES[s.routine];return(
        <div key={s.id} onClick={()=>setHistDetail(histDetail?.id===s.id?null:s)} style={{...card,marginBottom:8,cursor:"pointer",animation:`fadeUp 0.3s ease ${i*0.03}s both`,display:"flex",justifyContent:"space-between",alignItems:"center",background:histDetail?.id===s.id?T.bgEl:T.bgCard}}>
          <div><div style={{display:"flex",alignItems:"center",gap:8}}><span style={{fontSize:16}}>{r?.emoji}</span><span style={{fontSize:sz(14,fSc),fontWeight:700,color:T.w}}>{r?.name}</span></div>
          <div style={{fontSize:sz(11,fSc),color:T.t3,marginTop:3}}>{dateFR(s.date)} · {s.duration} · {s.exercises} exos</div></div>
          <div style={{display:"flex",alignItems:"center",gap:8}}>
            {s.prs>0&&<div style={{fontSize:sz(12,fSc),fontWeight:800,color:T.pk}}>🏆 {s.prs}</div>}
            {confirmDel===s.id?<div style={{display:"flex",gap:4}}>
              <button onClick={e=>{e.stopPropagation();deleteSession(s.id)}} style={{padding:"4px 8px",borderRadius:6,cursor:"pointer",background:"rgba(226,128,255,0.12)",border:"1px solid rgba(226,128,255,0.3)",color:T.pk,fontSize:10,fontWeight:700}}>Oui</button>
              <button onClick={e=>{e.stopPropagation();setConfirmDel(null)}} style={{padding:"4px 8px",borderRadius:6,cursor:"pointer",background:T.bgInput,border:`1px solid ${T.bd}`,color:T.t3,fontSize:10,fontWeight:700}}>Non</button>
            </div>:<button onClick={e=>{e.stopPropagation();setConfirmDel(s.id)}} style={{width:24,height:24,borderRadius:6,cursor:"pointer",background:"rgba(226,128,255,0.04)",border:"1px solid rgba(226,128,255,0.12)",color:T.t4,fontSize:10,display:"flex",alignItems:"center",justifyContent:"center",padding:0}}>✕</button>}
          </div>
        </div>)})}
      {histView==="routine"&&Object.entries(grouped).map(([rk,sessions],gi)=>{const r=INIT_ROUTINES[rk];return(
        <div key={rk} style={{marginBottom:16,animation:`fadeUp 0.3s ease ${gi*0.05}s both`}}>
          <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:8,padding:"0 4px"}}><span style={{fontSize:18}}>{r?.emoji}</span><span style={{fontSize:sz(15,fSc),fontWeight:800,color:T.w}}>{r?.name}</span><span style={{fontSize:sz(11,fSc),color:T.t3}}>({sessions.length} séances)</span></div>
          {sessions.map((s,i)=>(<div key={s.id} onClick={()=>setHistDetail(histDetail?.id===s.id?null:s)} style={{...card,marginBottom:6,cursor:"pointer",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
            ...
            <div style={{display:"flex",alignItems:"center",gap:8}}><span style={{fontSize:sz(11,fSc),color:T.t3}}>{s.exercises} exos</span>{s.prs>0&&<span style={{fontSize:sz(11,fSc),fontWeight:800,color:T.pk}}>🏆 {s.prs}</span>}</div></div>))}</div>)})}
  ...
  if(scr==="stats"){
    ...
    return(<div style={shell}><style>{css}</style>
      <div style={{padding:`calc(16px + ${safeTop}) 20px 16px`}}><Logo sz="sm"/><h2 style={{margin:"12px 0 0",fontSize:sz(22,fSc),fontWeight:800,color:T.w}}>Statistiques</h2></div>
      <div style={{display:"flex",padding:"0 16px",gap:6,marginBottom:16}}>
        {[{id:"prs",l:"🏆 PRs"},{id:"progress",l:"📈 Courbes"},{id:"volume",l:"💪 Volume"}].map(t=><button key={t.id} onClick={()=>{setSTab(t.id);setSelEx(null)}} style={btn(sTab===t.id)}>{t.l}</button>)}</div>
      <div style={{padding:"0 16px",paddingBottom:80}}>
        {sTab==="prs"&&<div>{prBoard.map((it,i)=>(<div key={it.id} onClick={()=>setSelEx(selEx===it.id?null:it.id)} style={{...card,marginBottom:8,cursor:"pointer",background:selEx===it.id?T.bgEl:T.bgCard,border:`1px solid ${selEx===it.id?T.bdM:T.bd}`,animation:`fadeUp 0.3s ease ${i*0.03}s both`}}>
        ...
        {sTab==="progress"&&<div>{selEx?(<div>
          ...
          <div style={{padding:"16px",borderRadius:16,...card}}>
            <div style={{fontSize:sz(16,fSc),fontWeight:800,color:T.w,marginBottom:4}}>{EX[selEx].name}</div>
            <div style={{fontSize:sz(11,fSc),color:T.t3,marginBottom:12}}>{EX[selEx].muscle} · {EX[selEx].hist.length} séances</div>
        ...
        </div>}</div><Nav active="stats" go={setScr}/></div>)}
  ...
  if(scr==="body"){
    ...
    const handleAddMensur=()=>{
      ...
      if(Object.keys(entry).length>1){const updated=DB.addMensuration(entry);setMensur(updated);setShowForm(false);setFormData({date:new Date().toISOString().slice(0,10),poids:"",bf:"",ventre:"",poitrine:"",cuisse:"",bras:"",muscle:"",meta:""})}
    };
    return(<div style={shell}><style>{css}</style>
        ...
        <div style={{padding:"16px",borderRadius:16,background:T.bgCard,border:`1px solid ${T.bdM}`}}>
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
            {MENSUR_FIELDS.map(f=>(<div key={f.key}><label style={{fontSize:10,color:T.t3,fontWeight:600,display:"block",marginBottom:3}}>{f.icon} {f.label} ({f.unit})</label>
              <input type="number" inputMode="decimal" step="0.1" placeholder={latest?.[f.key]?.toString()||""} value={formData[f.key]} onChange={e=>setFormData(p=>({...p,[f.key]:e.target.value}))} style={{width:"100%",padding:"9px 10px",borderRadius:9,background:T.bgInput,border:`1px solid ${T.bdM}`,color:T.w,fontSize:14,fontWeight:600,outline:"none",fontFamily:"inherit"}}/></div>))}
          </div>
      ...
      <div style={{display:"flex",padding:"0 16px",gap:6,marginBottom:16}}>
        {[{id:"dashboard",l:"📊 Vue"},{id:"history",l:"📋 Historique"}].map(t=><button key={t.id} onClick={()=>setBodyTab(t.id)} style={btn(bodyTab===t.id)}>{t.l}</button>)}</div>
      <div style={{padding:"0 16px",paddingBottom:80}}>
  ...
  /* ═══ SETTINGS ═══ */
  if(scr==="settings"){
    const setFS=(s)=>setSettings(p=>({...p,fontSize:s}));
    const setWG=(g)=>setSettings(p=>({...p,waterGoal:g}));
    const handleReset=()=>{DB.resetAll();setSwaps({});setCustomObjs({});setSessHist(INIT_HISTORY);setLastDates({...LAST_SESSIONS});setMensur([...INIT_MENSUR]);setSettings({fontSize:"normal",waterGoal:3000});setShowReset(false)};
    const lastExp=DB.getLastExport();
    const expDays=lastExp?Math.floor((new Date()-new Date(lastExp+"T12:00:00"))/(1000*60*60*24)):null;
    return(<div style={shell}><style>{css}</style>
      ...
      <div style={{padding:"0 16px",paddingBottom:80}}>
        ...
        <div style={{...card,marginBottom:10,animation:"fadeUp 0.3s ease both"}}>
          ...
          <div style={{display:"flex",gap:6}}>
            {[{id:"small",l:"Petit"},{id:"normal",l:"Normal"},{id:"large",l:"Grand"}].map(s=>(<button key={s.id} onClick={()=>setFS(s.id)} style={btn(settings.fontSize===s.id)}>{s.l}</button>))}</div></div>
        ...
        <div style={{...card,marginBottom:10,animation:"fadeUp 0.3s ease 0.05s both"}}>
          ...
          <div style={{display:"flex",gap:6}}>
            {[2000,2500,3000,3500].map(g=>(<button key={g} onClick={()=>setWG(g)} style={btn(waterGoal===g)}>{(g/1000).toFixed(1)}L</button>))}</div></div>
        ...
        <div style={{...card,marginBottom:10,animation:"fadeUp 0.3s ease 0.1s both"}}>
          ...
          {expDays==null&&<div style={{fontSize:11,color:T.wa,marginBottom:10,fontWeight:700}}>⚠️ Aucun export réalisé</div>}
          <div style={{display:"flex",gap:8}}>
            ...
            <label style={{flex:1,padding:"10px",borderRadius:10,cursor:"pointer",background:"rgba(168,140,255,0.08)",border:"1px solid rgba(168,140,255,0.2)",color:T.vi,fontSize:sz(12,fSc),fontWeight:700,textAlign:"center"}}>📥 Importer
              <input type="file" accept=".json" onChange={doImport} style={{display:"none"}}/></label>
          </div>
        </div>
        ...
        <div style={{...card,marginBottom:10,animation:"fadeUp 0.3s ease 0.15s both"}}>
          ...
          :<div style={{padding:"12px",borderRadius:10,background:"rgba(226,128,255,0.04)",border:"1px solid rgba(226,128,255,0.15)"}}>
            <div style={{fontSize:12,fontWeight:700,color:T.pk,marginBottom:10}}>⚠️ Es-tu sûr ? Action irréversible.</div>
            <div style={{display:"flex",gap:8}}>
              <button onClick={handleReset} style={{flex:1,padding:"10px",borderRadius:9,cursor:"pointer",background:"rgba(226,128,255,0.12)",border:"1px solid rgba(226,128,255,0.3)",color:T.pk,fontSize:13,fontWeight:700}}>Oui, tout supprimer</button>
