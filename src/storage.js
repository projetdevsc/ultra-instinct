...
export const DB = {
  ...
  // ─── Import routines from file (replaces all) ───
  importRoutinesFile: (data) => {
    // Add new exercises to custom exercises
    if (data.exercises) {
      const ce = DB.getCustomExercises();
      Object.entries(data.exercises).forEach(([id, ex]) => {
        ce[id] = ex;
      });
      store.set("customExercises", ce);
    }
    // Set objectives
    if (data.objectives) {
      const obj = DB.getObjectives();
      Object.entries(data.objectives).forEach(([id, val]) => {
        obj[id] = { mode: "custom", text: val };
      });
      store.set("objectives", obj);
    }
    // Replace routines
    if (data.routines) {
  ...
  // ─── Custom routines ───
  getRoutines: () => store.get("routines", null),
  setRoutines: (r) => store.set("routines", r),

  // ─── Import health data (macros + water from Apple Health) ───
  importHealth: (data) => {
    if (data.macros) {
      const existing = store.get("macros", {});
      const merged = { ...data.macros, ...existing }; // existing takes priority
      store.set("macros", merged);
    }
    if (data.water) {
      const existing = store.get("water", {});
      const merged = { ...data.water, ...existing };
      store.set("water", merged);
    }
  },
